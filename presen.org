...
About this informal short talk:
I wish to talk about the current limit of the language
(humbly and without sounding ignorant),
about macro systems (no, not about hygenic macros),
package systems and type systems.
...
** Iterate as =macrolet= ?
 ...
 + Useless: =for= in =body= are expanded *after* the =iter= macro
...
** Current solution: Code walker
 ...
 + Surrounding local macros are also expanded
