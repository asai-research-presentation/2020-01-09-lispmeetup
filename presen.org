...
*Import and export* : performed per each namespace

*Conflict* : happens within the same namespace

*Shadowing* : possible per each namespace (or for all namespaces, for convenience)
...
(square 5.0)            ; -> 25.0
(eq 'mypackage:square 'numeric:square) = (eq 'square 'square) ; -> T
(eq 'mypackage:square 'gui:square)     = (eq 'square 'square) ; -> T
#+end_src

(i.e. package prefixes are ignored)
